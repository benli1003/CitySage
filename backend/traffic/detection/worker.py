...
from .database_logger import log_vehicle_count, round_time_to_minute
...
def start_camera_worker(camera_id: str, stream_url: str):
    ...
    def on_prediction(data: dict, frame: VideoFrame):
        ...
        # logs car crossed per min into database
        if elapsed >= timedelta(minutes=1):
            ts = round_time_to_minute(now)
            count = active_counts[camera_id]

            try:
                log_vehicle_count(camera_id = camera_id, timestamp = ts, count = count)
                print(f"Logged {count} vehicles {ts}")
